...
calendar.x = awful.screen.focused().geometry.width / 2 - 140
calendar.y = -400
...
panel_timed.target = -400


function cal_toggle()
  if calendar.y <= -400 then
    panel_timed.target = beautiful.wibar_height
  ...
  if calendar.visible then
      ...
      calendar:set_widget(nil) -- just in case
      calendar:set_widget(calWidget)
  else
      awful.placement.top(calendar, { margins = { top = 40 }, parent = awful.screen.focused() })
